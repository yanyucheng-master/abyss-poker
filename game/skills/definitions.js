...
const SKILL_DEFINITIONS = Object.freeze({
  ...
  PERCEPTION: freezeSkill({
    ...
    expertDescription: "被动/完全秘密/情报。负载 3，费用 0。判定节点：底牌发完、Flop、Turn、River，四次独立判定，每手最多成功触发 3 次。触发概率随自身筹码劣势在 25%～50% 之间（公式参数化，未最终定稿）。每次成功获得一条信息：75% 真、25% 假。假信息必须选自当前真实状态下确实为假的命题，不能输出逻辑不可能或观察者已能直接证伪的信息。触发事件、内容与真假全部对对手隐藏。灵视只能发现“本手发生过感知事件”，不能看到具体内容与真假。绝密可以阻止感知访问受保护的底牌信息。",
  }),
